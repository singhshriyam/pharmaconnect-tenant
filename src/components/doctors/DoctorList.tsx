...
// Import shared types
import { DoctorListProps } from '../../components/common/types';
...
const DoctorList: React.FC<DoctorListProps> = ({ doctors, isLoading, onEdit, onDelete }) => {
  ...
  const filteredDoctors = doctors.filter(doc => {
    const searchMatch = doc.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
      doc.territory.toLowerCase().includes(searchTerm.toLowerCase()) ||
      doc.clinic_name.toLowerCase().includes(searchTerm.toLowerCase());
    const specialtyMatch = specialtyFilter === 'all' || doc.specialty === specialtyFilter;
    const potentialMatch = potentialFilter === 'all' || doc.potential === potentialFilter;
    return searchMatch && specialtyMatch && potentialMatch;
  });
      ...
      <div className="p-6">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search by name, territory, clinic..."
              value={searchTerm}
          ...
          <select
            ...
            className="px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white w-48"
          >
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200">
                ...
                <th className="text-left py-3 px-4 font-medium text-gray-700">Potential</th>
            ...
            <tbody>
              {filteredDoctors.map((doctor) => (
                <tr key={doctor.id} className="border-b border-gray-100 hover:bg-blue-50 transition-colors">
                  ...
                  <td className="py-4 px-4">
                    <span className="text-gray-900">{doctor.territory}</span>
                  </td>
                  <td className="py-4 px-4">
                    <span className={`px-2 py-1 rounded text-sm border ${getPotentialColor(doctor.potential)}`}>
                      {doctor.potential}
                    </span>
                  </td>
                  <td className="py-4 px-4">
                  ...
                  <td className="py-4 px-4">
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="p-2 hover:bg-red-100 rounded border border-red-300 text-red-600 transition-colors"
                      >
